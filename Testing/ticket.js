...
const { sep } = require('../config/myFun');
...
*extra note: can only delete the channel if you have admin permissions and the channel is active (ie: not revoked)*`,
  ...
  async run(msg, args) {
    if(!msg.guild.me.hasPermession()) msg.reply('I don\'t have the nessesary permissions!')
    ...
    const category = msg.guild.channels.find(role => role.name === 'tickets');

    if(!msg.guild.me.hasPermession()) msg.reply('I don\'t have the nessesary permissions!')
    // tickets is the enmap of tickets and config is enmap of guild settings.
...
please do \`${prefix}ticket [your comment on our service] --REVOKE\` here.`)
      ...
      },
      {
        ...
        .then(async (channel) => {
          ...
          channel.send(`${msg.author}\nIssue : ${msg.cleanContent.split(/ +/g).slice(1).join(' ')}`, TickEmbed).catch(console.error);
          if (msg.attachments.size) {
            for (const { url } of msg.attachments.values()) {
              request({ url, encoding: null }, async function(err, resp, data) {
                ...
                channel.send({ files: [new Discord.Attachment(base64data, 'file.png')] }).catch(console.error);
              });
            }
            await setTimeout(() => msg ? msg.delete() : false, 8000);
          }
          tickets.set(channel.id, { 'userId': msg.author.id, 'status': false });
        });
      setTimeout(() => msg ? msg.delete() : false, 900);
...
Please delete this channel. If the issue isn't resolved, please make a new ticket. Thank you!`,
      ...
      if (args[args.length - 1] === '--REVOKE'
        ...
        msg.channel.overwritePermissions(msg.author,
          { SEND_MESSAGES: false, ADD_REACTIONS: false, ATTACH_FILES: false, VIEW_CHANNEL: false }).catch(console.error);
        msg.channel.send(`Please do \`${prefix}ticket  --DELETE\` here to delete this channel and clear my records about this channel.`).catch(console.error);
      } else
      /* ~~~~~~ DELETE ~~~~~~ */
      ...
      if (args[args.length - 1] === '--REDELETE'
        ...
        msg.channel.fetchMessages({ before: msg.id })
          .then((messages) => {
            ...
            PartBlock = sep(PartBlock, 2000);
            Participants.forEach(Participant => PartBlock.forEach(part => Participant.send(`${part.replace('`', '\\`')}`, { code: ' ' }).catch(console.error)));
          },
          (error) => { console.log(error); });
